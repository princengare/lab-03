...
def save_mail(mail: List[Dict[str, str]]) -> None:
    ...
    Save the given mail list to the 'mail_db.json' file.

    Args:
        mail (List[Dict[str, str]]): A list of dictionaries, each representing an email message.
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Add a new mail entry to the mail list and return the unique ID of the entry.

    Args:
        mail_entry (Dict[str, str]): A dictionary representing an email message.

    Returns:
        str: A string representing the unique ID of the new mail entry.
    """
    mail = load_mail()
...
def delete_mail(mail_id: str) -> bool:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Delete the mail entry with the given ID from the mail list.

    Args:
        mail_id (str): A string representing the unique ID of the mail entry to be deleted.

    Returns:
        bool: True if the mail entry was found and deleted, False otherwise.
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Get the mail entry with the given ID from the mail list.

    Args:
        mail_id (str): A string representing the unique ID of the mail entry to retrieve.

    Returns:
        Optional[Dict[str, str]]: A dictionary representing the email message if found, or None if not found.
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Get all mail entries in the mail list that were sent to the given recipient.

    Args:
        recipient (str): A string representing the recipient's email address.

    Returns:
        List[Dict[str, str]]: A list of dictionaries, each representing an email message sent to the recipient.
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Get all mail entries in the mail list that were sent by the given sender.

    Args:
        sender (str): A string representing the sender's email address.

    Returns:
        List[Dict[str, str]]: A list of dictionaries, each representing an email message sent by the sender.
    """
    mail = load_mail()
    sent = []
    for entry in mail:
...
def get_sent_mail_route(sender: str):
    """
    Summary: Gets all mail entries sent by a sender from the json file
    ...
    Returns:
        list: A list of dictionaries representing the mail entries
    """
    res = jsonify(get_sent_mail(sender))
    res.status_code = 200
    return res
